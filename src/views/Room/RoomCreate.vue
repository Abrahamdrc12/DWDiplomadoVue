<template>
    <Layout>
        <div
            ...
            <form @submit.prevent="submit" class="space-y-6 w-1/2 p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 mb-4" :validation-schema="schema">
                <h5 class="text-xl text-center font-medium text-gray-900 dark:text-white">Crear Habitacion</h5>
                <div class="grid gap-6">
                    <div>
                        <label for="countries"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Hotel</label>
                        <select id="countries" v-model="hotel"
                    ...
                    <div>
                        <Button color="default" class="w-sm">Crear Habitacion</Button>
                    </div>
                </div>
            </form>
        </div>
...
watch([hotel, rtype, accom, cuartos], () => {
    fields.value = {
        hotel_id: parseInt(hotel.value),
        room_type_id: parseInt(rtype.value),
        accommodation_id: parseInt(accom.value),
...
const send = async (data) => {
    console.log(data)
    const link = "http://146.190.32.176/diplomado/api/rooms"
    axios.post(link , data).then((result) => {
        store.alert('success', 'Habitacion creada con exito')
    }).catch((err) => {
