<template>
    <Layout>
        <h1 class="text-center text-xl font-black mx-8 mb-4">Listado de habitaciones por acomodacion</h1>
...
const options = ref({
    ...
    xaxis: {
        categories: ["Sencilla", "Doble", "Triple", "Cuadruple"]
    }
});
...
const load = async () => {
    ...
    for (const item of data) {
            ...
            for (const j of results) {
                console.log(j)
                if (j.accommodation_id == 1){
                    sencilla+=1
                } else if (j.accommodation_id == 2){
                    doble+=1
                } else if (j.accommodation_id == 3){
                    triple+=1
                } else if (j.accommodation_id == 4){
                    cuadruple+=1
                }
            }
        })
    ...
    let newData = [sencilla, doble, triple, cuadruple]
